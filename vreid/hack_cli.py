...
def _ablate_postprocess(q, g, has_match, sims) -> dict:
    ...
    # 3/2 стоит первым не случайно: это параметры релиза, найденные полной сеткой
    # (scripts/rerank_grid.py, 93 точки, оптимум подтверждён на втором независимом кеше).
    # Прежняя сетка начиналась с 6/2 и не содержала строки релиза — из-за этого
    # scripts/scorecard.py не находил её в таблице и писал «пересчитай val».
    kr_grid = ((3, 2), (6, 2), (10, 3), (15, 4), (20, 6))
