...
def create_app(settings=None, core=None):
    ...
    def demo_info():
        """Небольшой набор настоящих кадров организаторов: 20 наблюдений и один запрос.

        Нужен, чтобы сервис можно было показать сразу после запуска, не имея под рукой
        датасета. Это проверка исполнения, а не измерение качества модели.
        """
        meta = settings.demo_dir / 'query.json'
        ...
        return {'available': True, 'image_url': '/v1/demo/query.jpg',
                ...
                'gallery_items': 20,
                'note': '20 кадров организаторов; демонстрация работы, не оценка точности'}
    ...
    def demo_gallery():
        ...
        if not archive.is_file():
            raise HTTPException(404, 'Демонстрационный архив не поставлен с этой сборкой.')
        if store.count():
