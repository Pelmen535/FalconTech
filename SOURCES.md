...
Вторая модель — та же линейка, `vit_large_patch14_dinov2.lvd142m`,
https://huggingface.co/timm/vit_large_patch14_dinov2.lvd142m, тот же автор и лицензия.
Она выполняет две роли: учителя при дистилляции (на обучении) и **ре-ранкера второй
ступени** (на инференсе, `release/reranker.pt`). В лимит 2 ГБ она входит — правила считают
его по сумме весов «основного backbone, auxiliary-моделей, классификаторов, ensemble и
reranker» (ответы 34, 37).

**Важно для проверки лимита 2 ГБ.** В релизном образе лежат два файла весов:

| файл | что это | размер |
|---|---|---:|
| `release/model.pt` | ViT-B/14 336, основная модель, её вектор идёт в `embeddings.npy` и её путь меряет жюри | 172 МБ |
| `release/reranker.pt` | ViT-L/14 336, уточняет порядок внутри топ-30 | 608 МБ |
| | **сумма против лимита 2 000 000 000 байт** | **780 МБ** |

Сумма проверяется не на словах: `vreid.artifacts.weight_inventory` считает её при сборке
образа и падает при превышении, а `scripts/export_release.py` проверяет то же самое ещё на
экспорте — чтобы узнать о превышении до сборки, а не после.

Оба файла самодостаточны: архитектура собирается timm с `pretrained=False`, из сети в
рантайме ничего не тянется (`HF_HUB_OFFLINE=1`, `TRANSFORMERS_OFFLINE=1`). Промежуточные
чекпойнты и веса остальных учителей в образ не попадают.
...
## 2.5 Материалы организаторов

Получены от организаторов 21.09.2026 и лежат в `organizer/` без изменений — их нельзя
считать нашим кодом, но на них опирается всё, что мы утверждаем про метрику.

| файл | размер | SHA-256 |
|---|---:|---|
| `organizer/evaluate.py` | 17 160 Б | `655c71db8c2e4d2cd7680c40c768afacfdffff360401111c1a46df921551ffa3` |
| `organizer/example_submission.zip` | 2 097 Б | `cccda5bf0c801be8fd5748646dac345f180f6d11988ecbf4ec17ac9c311e4f46` |
